...
import { Search, Edit, Eye, Trash2, ArrowUpDown, UserPlus, Coins } from "lucide-react";
...
export function MembersTable({ 
  ...
  const getPointsBadgeColor = (points: number) => {
    if (points >= 200) return "bg-success bg-opacity-10 text-success";
    if (points >= 100) return "bg-primary bg-opacity-10 text-primary";
    if (points >= 50) return "bg-warning bg-opacity-10 text-warning";
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
            ...
            <Button className="inline-flex items-center" data-testid="button-add-member">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {paginatedMembers.map((member) => (
                    <tr key={member.id} className="hover:bg-gray-50" data-testid={`member-row-${member.id}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge 
                          className={getPointsBadgeColor(member.puntos)}
                          data-testid={`member-points-${member.id}`}
                        >
                  ...
                  {paginatedMembers.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                        {searchQuery ? "No se encontraron socios que coincidan con la búsqueda" : "No hay socios registrados"}
                      </td>
                    </tr>
                  )}
                </tbody>
              ...
              <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-700" data-testid="pagination-info">
                    ...
                    <span className="font-medium">{Math.min(startIndex + itemsPerPage, sortedMembers.length)}</span> de{" "}
                    <span className="font-medium">{sortedMembers.length}</span> resultados
                  </div>
                  <div className="flex space-x-2">
                    <Button
                      variant="outline"
